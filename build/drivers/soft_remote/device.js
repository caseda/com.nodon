	...
	async onNodeInit() {
			...
			if (
				...
				if (this.getSetting('sequence_activation') && report.Properties1['Key Attributes'] === 'Key Pressed 1 time') {
					if (!previousSequence || previousSequence + 1 === report['Sequence Number']) {
						...
						this._softRemoteScene.trigger(this, null, remoteValue).catch(this.error);
						this._softRemoteSceneToken.trigger(this, remoteValue, null).catch(this.error);
					...
					sequenceTimeout = setTimeout(() => {
						...
						} else this._softRemoteSequence.trigger(this, null, { sequence: sequence }).catch(this.error);
		...
		this.registerReportListener('BASIC', 'BASIC_SET', report => {
			if (report.Value !== undefined) {
				if (report.Value > 0) this._softRemoteNonScene.trigger(this, null, { value: 'on' }).catch(this.error);
				else this._softRemoteNonScene.trigger(this, null, { value: 'off' }).catch(this.error);
			}
		});
		...
		this.registerReportListener('SWITCH_MULTILEVEL', 'SWITCH_MULTILEVEL_START_LEVEL_CHANGE', report => {
			if (
				...
				if (report.Level['Up/ Down']) this._softRemoteNonScene.trigger(this, null, { value: 'down' }).catch(this.error);
				else this._softRemoteNonScene.trigger(this, null, { value: 'up' }).catch(this.error);
			}
		});

		this.registerReportListener('SWITCH_MULTILEVEL', 'SWITCH_MULTILEVEL_STOP_LEVEL_CHANGE', report => {
			this._softRemoteNonScene.trigger(this, null, { value: 'stop' }).catch(this.error);
		});
	}
