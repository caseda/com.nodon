	...
	if (node && typeof node.instance.CommandClass.COMMAND_CLASS_CENTRAL_SCENE !== 'undefined') {
		node.instance.CommandClass.COMMAND_CLASS_CENTRAL_SCENE.on('report', (command, report) => {
			if (command &&
				...
				Homey.manager('flow').triggerDevice('soft_remote', null, remote_value, node.device_data);
			}
		});
	}
	...
	if (node && typeof node.instance.CommandClass.COMMAND_CLASS_BASIC !== 'undefined') {
		node.instance.CommandClass.COMMAND_CLASS_BASIC.on('report', (command, report) => {
			if (command &&
				...
				if(report.Value === 255) Homey.manager('flow').triggerDevice('soft_remote_nonscene', null, { value: 'on' }, node.device_data);
				if (report.Value === 0) Homey.manager('flow').triggerDevice('soft_remote_nonscene', null, { value: 'off' }, node.device_data);
			}
		});
	}
	...
	if (node && typeof node.instance.CommandClass.COMMAND_CLASS_SWITCH_MULTILEVEL !== 'undefined') {
		node.instance.CommandClass.COMMAND_CLASS_SWITCH_MULTILEVEL.on('report', (command, report) => {
			if (command &&
				...
				if (report.Level['Up/ Down']) Homey.manager('flow').triggerDevice('soft_remote_nonscene', null, { value: 'down' }, node.device_data);
				else Homey.manager('flow').triggerDevice('soft_remote_nonscene', null, { value: 'up' }, node.device_data);
			}
			if (command.name === 'SWITCH_MULTILEVEL_STOP_LEVEL_CHANGE') Homey.manager('flow').triggerDevice('soft_remote_nonscene', null, { value: 'stop' }, node.device_data);
		});
	}
});

Homey.manager('flow').on('trigger.soft_remote', (callback, args, state) => {
	if(state &&
...
Homey.manager('flow').on('trigger.soft_remote_nonscene', (callback, args, state) => {
	if(state &&
