...
class WallSwitch extends ZwaveDevice {
	async onNodeInit() {
		...
		// Register
		this._wallSwitchScene = this.driver.wallSwitchScene;
		this._wallSwitchSceneToken = this.homey.app.sceneTokenRegister;
		this._wallSwitchNonScene = this.driver.wallSwitchNonScene;
		this._wallSwitchSequence = this.homey.app.sequenceRegister;
			...
			if (
				...
				if (this.getSetting('sequence_activation') && report.Properties1['Key Attributes'] === 'Key Pressed 1 time') {
					...
					if (!previousSequence || previousSequence + 1 === report['Sequence Number']) {
						...
						this._wallSwitchScene.trigger(this, null, remoteValue).catch(this.error);
						this._wallSwitchSceneToken.trigger(this, remoteValue, null).catch(this.error);
					...
					sequenceTimeout = setTimeout(() => {
						...
						} else this._wallSwitchSequence.trigger(this, null, { sequence: sequence }).catch(this.error);
		...
		this.registerReportListener('BASIC', 'BASIC_SET', report => {
			if (report.Value !== undefined) {
				if (report.Value > 0) this._wallSwitchNonScene.trigger(this, null, { value: 'on' }).catch(this.error);
				else this._wallSwitchNonScene.trigger(this, null, { value: 'off' }).catch(this.error);
			}
		});
		...
		this.registerReportListener('SWITCH_MULTILEVEL', 'SWITCH_MULTILEVEL_START_LEVEL_CHANGE', report => {
			if (
				...
				if (report.Level['Up/ Down']) this._wallSwitchNonScene.trigger(this, null, { value: 'down' }).catch(this.error);
				else this._wallSwitchNonScene.trigger(this, null, { value: 'up' }).catch(this.error);
			}
		});

		this.registerReportListener('SWITCH_MULTILEVEL', 'SWITCH_MULTILEVEL_STOP_LEVEL_CHANGE', report => {
			this._wallSwitchNonScene.trigger(this, null, { value: 'stop' }).catch(this.error);
		});
	}

	// Flows Run Listeners
	async wallSwitchSceneRunListener(args, state) {
		if (
	...
	async wallSwitchNonSceneRunListener(args, state) {
		if (
