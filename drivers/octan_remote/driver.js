	...
	if (node && typeof node.instance.CommandClass.COMMAND_CLASS_BASIC !== 'undefined') {
		node.instance.CommandClass.COMMAND_CLASS_BASIC.on('report', (command, report) => {
			if (command &&
				...
				if(report.Value === 255) Homey.manager('flow').triggerDevice('octan_remote_nonscene', null, { value: 'on' }, node.device_data);
				if (report.Value === 0) Homey.manager('flow').triggerDevice('octan_remote_nonscene', null, { value: 'off' }, node.device_data);
			}
		});
	}
	...
	if (node && typeof node.instance.CommandClass.COMMAND_CLASS_SWITCH_MULTILEVEL !== 'undefined') {
		node.instance.CommandClass.COMMAND_CLASS_SWITCH_MULTILEVEL.on('report', (command, report) => {
			if (command &&
				...
				if (report.Level['Up/ Down']) Homey.manager('flow').triggerDevice('octan_remote_nonscene', null, { value: 'down' }, node.device_data);
				else Homey.manager('flow').triggerDevice('octan_remote_nonscene', null, { value: 'up' }, node.device_data);
			}
			if (command.name === 'SWITCH_MULTILEVEL_STOP_LEVEL_CHANGE') Homey.manager('flow').triggerDevice('octan_remote_nonscene', null, { value: 'stop' }, node.device_data);
		});
	}
});
