	...
	if (typeof node.instance.CommandClass.COMMAND_CLASS_BASIC !== 'undefined') {
		node.instance.CommandClass.COMMAND_CLASS_BASIC.on('report', (command, report) => {
			...
			if (command.name === 'BASIC_SET' && typeof report.Value !== 'undefined') {
				if(report.Value === 255) {
					Homey.manager('flow').triggerDevice('wall_switch_nonscene', null, { value: 'on' }, node.device_data);
				}
				if (report.Value === 0) {
					Homey.manager('flow').triggerDevice('wall_switch_nonscene', null, { value: 'off' }, node.device_data);
				}
			}
		});
	}
		...
		node.instance.CommandClass.COMMAND_CLASS_SWITCH_MULTILEVEL.on('report', (command, report) => {
			...
			if (command.name === 'SWITCH_MULTILEVEL_START_LEVEL_CHANGE' && typeof report.Level['Up/ Down'] === 'boolean') {
				if (report.Level['Up/ Down']) {
					Homey.manager('flow').triggerDevice('wall_switch_nonscene', null, { value: 'down' }, node.device_data);
				} else {
					Homey.manager('flow').triggerDevice('wall_switch_nonscene', null, { value: 'up' }, node.device_data);
				}
			}

			if (command.name === 'SWITCH_MULTILEVEL_STOP_LEVEL_CHANGE') {
				Homey.manager('flow').triggerDevice('wall_switch_nonscene', null, { value: 'stop' }, node.device_data);
			}
		});
	}
});
